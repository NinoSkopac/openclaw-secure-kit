...
function buildFirewallRules(profileName: string, profile: ProfileConfig): string {
  const hardenedProxyOnly = profile.network.hardened_egress_mode === "proxy-only";
  const allowPorts = normalizedAllowPorts(profile);
  const allowedPortRules = hardenedProxyOnly
    ? allowPorts.map((port) => `    ip saddr ${OPENCLAW_EGRESS_PROXY_IP} tcp dport ${port} accept`).join("\n")
    : allowPorts.map((port) => `    ip saddr ${OPENCLAW_NETWORK_SUBNET} tcp dport ${port} accept`).join("\n");
  const egressRules = hardenedProxyOnly
    ? `
    # Hardened mode: openclaw services can only egress through local proxy.
    ip saddr ${OPENCLAW_NETWORK_SUBNET} ip daddr ${OPENCLAW_EGRESS_PROXY_IP} tcp dport ${OPENCLAW_EGRESS_PROXY_PORT} accept
${allowedPortRules}
`
    : `
...
export function applyFirewall(profileName: string): void {
  ...
  fs.writeFileSync(OPENCLAW_FIREWALL_RULES_PATH, buildFirewallRules(profileName, profile), "utf8");
