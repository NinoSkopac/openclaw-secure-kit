...
const HardenedEgressModeSchema = z.enum(["disabled", "proxy-only"]);
...
const NetworkSchema = z
  .object({
    ...
    hardened_egress_mode: HardenedEgressModeSchema.default("disabled"),
    proxy_only_egress: z.boolean().optional()
  })
  .default({})
  .transform((network) => ({
    ...
    hardened_egress_mode:
      network.proxy_only_egress === true ? "proxy-only" : network.hardened_egress_mode
  }));
